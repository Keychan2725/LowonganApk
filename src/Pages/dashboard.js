...
export default function Dashboard() {
  ...
  const [pegawaiModal, setPegawaiModal] = React.useState(false);
  const [selectedDetail, setSelectedDetail] = useState(null);
  const id = localStorage.getItem("userId");

  const [pegawai, setPegawai] = useState([]);
  const [pegawaiById, setPegawaiById] = useState([]);
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/pegawai/all`).then((res) => {
      setPegawai(res.data);
    });
  }, []);
  useEffect(() => {
    axios.get(`http://localhost:8080/api/pegawai/${id}`).then((res) => {
      setPegawaiById(res.data);
    });
  }, []);
  ...
  const showPegawaiDetailModal = (detail) => {
    setSelectedDetail(detail);
    setPegawaiModal(true);
  };
  return (
    <>
      <Sidebar />
        ...
        <div className="flex justify-center w-[100%]">
          <main className="s-content w-[390px] md:w-[1125px]    md:px-10 mx-8 ">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col bg-white m-auto p-auto pekerjaan-container">
            ...
            <div className="grid grid-cols-1 mr-24 md:grid-cols-2 gap-4">
              <div className="flex flex-col bg-white m-auto p-auto pegawai-container">
                <h3 className="flex py-5 lg:px-20 md:px-10 px-5 lg:mx-40 md:mx-20 mx-5 font-bold text-2xl text-gray-800">
                  Pegawai
                </h3>
                <div className="flex overflow-x-scroll pb-10 hide-scroll-bar">
                  {pegawai.map((pegawai) => (
                    <div className="flex flex-nowrap lg:ml-12 md:ml-12 ml-5">
                      <div className="inline-block px-3 mb-1 relative border border-gray-300">
                        <img
                          className="w-64 h-64 my-2 max-w-xs overflow-hidden rounded-lg shadow-md"
                          src={pegawai.fotoPegawai}
                          alt=""
                        />

                        <div className="block p-4">
                          <h3 className="text-xl font-bold mb-2 text-gray-800  ">
                            {pegawai.inginBekerjaDi}
                          </h3> 
                          <p className="text-gray-600  ">
                            {pegawai.namaPegawai}
                          </p>
                        ...
                        <div className="flex justify-around items-center mt-2">
                          <button
                            className="w-24 h-8 bg-blue-500 text-white active:bg-blue-200 font-bold uppercase text-sm rounded shadow hover:shadow-lg outline-none focus:outline-none  ease-linear transition-all duration-150"
                            onClick={() => showPegawaiDetailModal(pegawai.id)}
                          >
                            Detail
                          </button>
                          <a
                          href="/rekrut-pegawai"
                            className="w-24 h-8 text-center rounded-lg text-white my-5 font-bold uppercase justify-self-end bg-lime-400 hover:bg-gray-300 hover:text-dark"
                          >
                            Rekrut
                          </a>
                        </div>
              ...
              <div className="relative w-full md:w-auto my-6 mx-auto max-w-3xl">
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gradient-to-b from-black to-white border-solid border-blueGray-200 outline-none focus:outline-none">
                  ...
                  <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-lg">
                    <h3 className="text-3xl font-semibold text-white">Detail</h3>
                    <button
                  ...
                  <div className="relative p-6 flex-auto">
                    <div className="flex flex-col md:flex-row items-center md:items-start justify-center md:justify-around">
                      <div className="mb-4 md:mb-0">
                        <img src="" alt="" className="w-64 h-64 max-w-xs rounded-lg" />
                      </div>
                      <div className="md:mx-5">
                        ...
                        <p className="text-xl text-white">Kontak: 086356237</p>
                        <p className="text-xl text-white">Alamat: Semarang, Mangkang kanan</p>
                        <p className="text-xl text-white">Permintaan Gaji: 2 T</p>
                        <p className="text-xl text-white">Pengalaman Bekerja: Pernah Bekerja Di Pasar Jumat</p>
                        <p className="text-xl text-white">Ingin Bekerja Di: PT Telkomsel Indonesia</p>
                      </div>
                    </div>
                  </div>
                  {/*footer*/}
